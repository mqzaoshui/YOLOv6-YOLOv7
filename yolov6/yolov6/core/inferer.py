...
class Inferer:
    ...
    #xyh save the infer time 2 txt
    def save_time2txt(self,line,file):
        with open(file,'a') as f:
            f.write(line + '\n') 
    # xyh pytorch-accurate time
    def time_synchronized(self):
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        return time.time() 

    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, save_time, view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            ...
            #xyh save time
            if save_time:
                time2txt = osp.join(save_dir,'time.txt')
                pred_time = round(1E3*(t2 - t1), 2)
                nms_time = round(1E3*(t3 - t2),2)
                time = [pred_time,nms_time]
                line = str(time)
                self.save_time2txt(line, time2txt)
